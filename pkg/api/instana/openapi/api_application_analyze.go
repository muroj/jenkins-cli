...
func (a *ApplicationAnalyzeApiService) GetCorrelatedTraces(ctx _context.Context, correlationId string) ([]BackendTraceReference, *_nethttp.Response, error) {
	...
	// create path and map variables
	localVarPath := a.client.cfg.BasePath + "/api/application-monitoring/analyze/backend-correlation"
	localVarHeaderParams := make(map[string]string)
	localVarQueryParams := _neturl.Values{}
	localVarFormParams := _neturl.Values{}
	...
	if strlen(correlationId) > 128 {
		return localVarReturnValue, nil, reportError("correlationId must have less than 128 elements")
	}

	localVarQueryParams.Add("correlationId", parameterToString(correlationId, ""))
	// to determine the Content-Type header
